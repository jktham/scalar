...
pub fn create_sparks_effect(
    ...
    gradient.add_key(0.5, Vec4::new(1.0, 0.0, 0.0, 0.8));
    ...
    let y = writer.lit(-5.0).uniform(writer.lit(5.0));
    let x = writer.lit(-5.0).uniform(writer.lit(5.0));
    let z = writer.lit(-5.0).uniform(writer.lit(5.0));
    let v = x.clone().vec3(y, z);
    let init_vel = SetAttributeModifier::new(Attribute::VELOCITY, v.expr());
    ...
    let s = writer.lit(0.1);
    let init_size = SetAttributeModifier::new(Attribute::SIZE, s.expr());

    let update_gravity = SetAttributeModifier::new(
        Attribute::VELOCITY,
        writer
            .attr(Attribute::VELOCITY)
            .sub(
                writer
                    .lit(0.0)
                    .vec3(writer.lit(16.0).mul(writer.delta_time()), writer.lit(0.0)),
            )
            .expr(),
    );

    let name = "sparks";
    let effect = EffectAsset::new(100, SpawnerSettings::rate(160.0.into()), writer.finish())
        .with_name(name)
        .init(init_pos)
        .init(init_rot_x)
        .init(init_rot_y)
